...
def convert_scienceqa_to_hf():
    ...
    # Split into test and train, ensuring both are divisible by 8
    target_test_size = 1600
    
    # Calculate actual sizes that are divisible by 8
    test_size = (target_test_size // 8) * 8  # Round down to nearest multiple of 8
    remaining_data = len(all_examples) - test_size
    train_size = (remaining_data // 8) * 8  # Round down to nearest multiple of 8
    
    # Only use the amount of data we can evenly divide
    total_used = test_size + train_size
    
    test_examples = all_examples[:test_size]
    train_examples = all_examples[test_size:test_size + train_size]
    
    print(f"Split into (ensuring divisible by 8):")
    print(f"  Test: {len(test_examples)} examples (divisible by 8: {len(test_examples) % 8 == 0})")
    print(f"  Train: {len(train_examples)} examples (divisible by 8: {len(train_examples) % 8 == 0})")
    print(f"  Total used: {total_used} / {len(all_examples)} examples")
    print(f"  Discarded: {len(all_examples) - total_used} examples")
