...
class VILAPretrainedModel(PreTrainedModel):
    ...
    def convert_vila_dev_ckpt_to_remote(
        ...
        **kwargs,
        ...
        # assert type(self) == VILAForCausalLM, "This method is only available for VILAForCausalLM."
    ...
    def __embed_media_tokens(
        ...
        for name in media:
            ...
            if "frames_split" in media_config[name]:
                frames_split = media_config[name].pop("frames_split")

            if frames_split > 0 and "video" in name:
                media_split = []
                for a in media[name]:
                    media_split += a.to(torch.bfloat16).tensor_split(frames_split, dim=0)
                embeds_split = []
                for a in media_split:
                    embeds_split += self.encoders[name]([a], media_config[name])
